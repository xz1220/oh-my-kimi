...
Use the CLI-first state surface (`omk state ... --json`) for ultrawork lifecycle state. If explicit MCP compatibility tools are already available, equivalent `omx_state` calls are optional compatibility, not the default.
...
- **On cancellation/cleanup**:
  run `$cancel` (which should call `omk state clear --input '{"mode":"ultrawork"}' --json`)
