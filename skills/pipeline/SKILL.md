...
| `workerCount` | 2 | Number of Kimi CLI team workers |
...
- **team-exec**: Team execution via Kimi CLI workers. Always the oh-my-kimi execution backend.
...
- **On start**: `omk state write --input '{"mode":"pipeline","active":true,"current_phase":"stage:ralplan"}' --json`
- **On stage transitions**: `omk state write --input '{"mode":"pipeline","current_phase":"stage:<name>"}' --json`
- **On completion**: `omk state write --input '{"mode":"pipeline","active":false,"current_phase":"complete"}' --json`
...
- **team**: Pipeline delegates execution to team mode (Kimi CLI workers)
